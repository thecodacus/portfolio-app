          ...
          <div className="mb-12">
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-3">
                {technicalSkills
                  .filter(skill => 
                    ['Python', 'JavaScript/TypeScript', 'C#', 'C++', 'Angular', 'Node.js', 'AWS Lambda', 'Docker'].includes(skill.name)
                  )
                  .map(skill => (
                    <span 
                      key={skill.id} 
                      className="bg-gray-100 text-gray-800 px-4 py-2 rounded-full hover:bg-blue-100 hover:text-blue-800 transition-colors duration-300"
                    >
                      {skill.name}
                    </span>
                  ))
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-3">
                {technicalSkills
                  .filter(skill => 
                    ['Machine Learning', 'SK Learn', 'Pandas', 'Numpy', 'Tensorflow', 'PyTorch', 'Keras', 
                     'Linear Regression', 'Logistic Regression', 'Decision Tree', 'Boosting', 'KNN', 
                     'K-mean clustering', 'Naive Bayes'].includes(skill.name)
                  )
                  .map(skill => (
                    <span 
                      key={skill.id} 
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {technicalSkills
                  .filter(skill => 
                    ['Serverless & Cloud', 'AWS Lambda', 'Docker', 'Software Development'].includes(skill.name)
                  )
                  .map(skill => (
                    <span 
                      key={skill.id} 
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {softSkills.map(skill => (
                <span 
                  key={skill.id} 
                  className="bg-blue-50 text-blue-800 px-4 py-2 rounded-full hover:bg-blue-100 transition-colors duration-300"
                >
                  {skill.name}
                </span>
              ))}
